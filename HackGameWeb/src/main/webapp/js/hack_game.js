...
function initGame() {
	...
	xhttp.onreadystatechange = function() {
		if (xhttp.readyState == 4 && xhttp.status == 200) {
			...
			console.log("INIT GAME COORDZZ "+player1x+" "+player1y+" , "+player2x+" "+player2y);
...
function initRender(player1x, player1y, player2x, player2y, trapList) {
	...
	//var trapElement = "rect" + 10 + 5;
	//var trapCell = document.getElementById(trapElement);
	...
	for (var i=0; i<trapList.length; i++)
	{
		...
		//{
		trapIterElement += trapList[i][0];
		if (trapList[i][0] == 11 && trapList[i][1] < 6) {
			trapIterElement = trapIterElement + 0 +trapList[i][1];
			console.log("Fixing ambiguous traps to " + trapIterElement)
		}
		else {
			trapIterElement += trapList[i][1];
		}
		//}
		console.log("Rendering trap for cell "+trapIterElement);
